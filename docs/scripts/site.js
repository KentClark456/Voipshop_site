// docs/scripts/site.js — universal full-bleed navbar (kills legacy navs)
(function () {
...
// Lock fonts/spacing and prevent page containers from constraining the nav
function injectNavLock() {
  if (document.querySelector('style[data-nav-style-lock]')) return;
  const css = `
    /* Isolate the header from page-level typography/layout utilities */
    ...
    /* Inner wrapper must never be centered/narrowed by page styles */
    #nav-slot > header > div{
      margin:0 !important; width:100% !important;
      display:flex !important; align-items:center !important; justify-content:space-between !important; gap:24px !important;
      padding:12px 24px 12px 12px !important;   /* logo slightly closer to the left */
    }

    /* Typography & link styling (override tracking/centering from pages) */
    ...
    /* Ensure inline SVGs render crisply */
...
// The nav itself: logo left, menu right (full-width inner wrapper)
const NAV_HTML = `
<header style="
...
<a href="checkout.html" title="Cart"
  ...
  ensureTailwind().finally(() => {
    injectNavCSS();
    injectNavLock();
    removeLegacyNav();                // ← ensure old navs are gone
